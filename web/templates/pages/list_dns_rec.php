...
<div class="toolbar">
    ...
    <div class="toolbar-inner">
        <div class="toolbar-buttons">
            ...
            <?php if ($read_only !== "true") { ?>
                <?php $domain_q = tohtml(http_build_query(array("domain" => $_GET["domain"]))); ?>
                <a href="/add/dns/?<?= $domain_q ?>" class="button button-secondary js-button-create">
                    <i class="fas fa-circle-plus icon-green"></i><?= _("Add Record") ?>
                </a>
                <a href="/edit/dns/?<?= $domain_q ?>" class="button button-secondary js-button-create">
                    <i class="fas fa-pencil icon-blue"></i><?= _("Edit DNS Domain") ?>
                </a>
            <?php } ?>
        </div>
        <div class="toolbar-right">
            <div class="toolbar-sorting">
                ...
                <ul class="toolbar-sorting-menu js-sorting-menu u-hidden">
                    <li data-entity="sort-date" data-sort-as-int="1">
                        <?php
                        $sort_date_active_class = (isset($_SESSION['userSortOrder']) && $_SESSION['userSortOrder'] === 'date')
                            ? 'active'
                            : '';
                        ?>
                        <span class="name <?= $sort_date_active_class ?>">
                            <?= _("Date") ?> <i class="fas fa-arrow-down-a-z"></i>
                        </span>
                        <span class="up"><i class="fas fa-arrow-up-a-z"></i></span>
                    </li>
...
<div class="container">
    ...
    foreach ($data as $key => $value) {
        ...
        <div class="units-table-row <?php if ($status == 'suspended') {
            ...
            <div class="units-table-cell">
                <div>
                    <?php $rec_id_html = tohtml($data[$key]["ID"]); ?>
                    <input
                        id="check<?= $rec_id_html ?>"
                        class="js-unit-checkbox"
                        type="checkbox"
                        title="<?= _("Select") ?>"
                        name="record[]"
                        value="<?= $rec_id_html ?>"
                        <?= $display_mode ?>>
                    <label for="check<?= $rec_id_html ?>" class="u-hide-desktop"><?= _("Select") ?></label>
                </div>
            </div>
            <div class="units-table-cell units-table-heading-cell u-text-bold">
                <span class="u-hide-desktop"><?= _("Record") ?>:</span>
                <?php if (($read_only === 'true') || ($data[$key]['SUSPENDED'] == 'yes')) { ?>
                    <?= tohtml(substr($data[$key]['RECORD'], 0, 12));
                    if (strlen($data[$key]['RECORD']) > 12) {
                        echo '...';
                    } ?>
                <?php } else { ?>
                    <?php
                    $edit_query = tohtml(http_build_query(array(
                        "domain" => $_GET['domain'],
                        "record_id" => $data[$key]['ID'],
                        "token" => $_SESSION['token'],
                    )));
                    $edit_title = tohtml(_("Edit DNS Record") . ': ' . $data[$key]['RECORD']);
                    ?>
                    <a href="/edit/dns/?<?= $edit_query ?>" title="<?= $edit_title ?>">
                        <?= tohtml(substr($data[$key]['RECORD'], 0, 12));
                        if (strlen($data[$key]['RECORD']) > 12) {
                            echo '...';
                        } ?>
                        ...
                        <?php if ($read_only !== "true") { ?>
                            <?php if ($data[$key]["SUSPENDED"] == "no") { ?>
                                <li class="units-table-row-action shortcut-enter" data-key-action="href">
                                    <a
                                        class="units-table-row-action-link"
                                        href="/edit/dns/?<?= $action_edit_query ?>"
                            ...
                            <li class="units-table-row-action shortcut-delete" data-key-action="js">
                                <?php
                                $delete_query = tohtml(http_build_query(array(
                                    "domain" => $_GET["domain"],
                                    "record_id" => $data[$key]["ID"],
                                    "token" => $_SESSION["token"],
                                )));
                                $action_edit_query = tohtml(http_build_query(array(
                                    "domain" => $_GET["domain"],
                                    "record_id" => $data[$key]["ID"],
                                    "token" => $_SESSION["token"],
                                )));
                                $delete_confirm = tohtml(sprintf(_("Are you sure you want to delete record %s?"), $key));
                                ?>
                                <a
                                    class="units-table-row-action-link data-controls js-confirm-action"
                                    href="/delete/dns/?<?= $delete_query ?>"
                                    title="<?= _("Delete") ?>"
                                    data-confirm-title="<?= _("Delete") ?>"
